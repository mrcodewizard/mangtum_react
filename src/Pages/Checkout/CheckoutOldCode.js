...
import { userCheckout } from "../../store/checkout";
import CartItem from "../Cart/CartItem";
import { toast } from "react-toastify";

const CheckoutOldCode = () => {
  ...
  const cartStatus = useSelector((state) => state.checkout);

  const [cartItems, setCartItems] = useState([]);
  const [updated, setUpdated] = useState(false);
  const [prodQty, setProdQty] = useState(1);

  const mycartLists = useSelector((state) => state.checkout.checkout_data);
...
export default CheckoutOldCode;
